...
class GatewayService:
    ...
    def _prompt_for_message(self, content: str) -> str:
        if content in {"/总结", "/summary"}:
            return (
                "只读取个人探索工作空间中的个人探索档案，简明总结学习进度、学习项目、"
                "部署项目、个人兴趣、主要成果、风险和未来两周最重要的行动。"
            )
        if content in {"/任务", "/tasks"}:
            return (
                "只读取个人探索档案/05_行动清单.md 和最新记录，列出当前任务，按优先级说明"
        ...
        if content.startswith("/记录 ") or content.startswith("/record "):
            ...
            return (
                ...
                "待确认内容和分析判断，再更新个人探索档案中相关的原始记录、学习项目、"
                "部署项目、兴趣探索和行动清单，最后说明更新了什么以及下一步建议。不要读取或修改工作档案。\n\n"
                f"新记录：\n{record}"
            )
        return content

    async def _run_codex(self, user_id: str, prompt: str) -> None:
        self.store.set_status(user_id, "analyzing", "已收到，准备读取个人探索档案")
        await self._reply(user_id, "已收到，正在读取个人探索档案并分析。")
    ...
    async def _ensure_thread(self, user_id: str) -> str:
        ...
        if session.thread_id:
            if session.thread_id not in self._loaded_threads:
                ...
                return session.thread_id
        thread_id = await self.codex.start_thread(f"个人探索分析 - {user_id}")
        self.store.set_thread(user_id, thread_id)
        self._loaded_threads.add(thread_id)
        return thread_id
